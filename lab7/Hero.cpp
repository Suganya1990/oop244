...
namespace sict {
	...
	Hero::Hero(const char* name, int numHealth, int numAttackStenght)
	{
		//assigns valid data into my member functions
		if (name != nullptr && name[0] != '\0' && numHealth != 0 && numAttackStenght != 0)
		{
			//copys name into object m_name
			strncpy(m_name, name, max_hero_name - 1);
			m_name[max_hero_name] = '\0';
			m_health = numHealth;
			m_attachStrenght = numAttackStenght;
	...
	{
		if (m_name[0] == '\0' && m_health == 0 && m_attachStrenght)

			return 0;
		else
			return m_attachStrenght;
	}
